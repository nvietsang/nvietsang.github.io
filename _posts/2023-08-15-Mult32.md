...
Suppose that we are given two positive numbers $$x,y \in [0, 2^{32}-1]$$, the goal is to calculate $$z = x \times y$$ where $$z \in [0, 2^{64} - 1]$$ without using 64-bit any variable (and also, without using conditional expressions).
